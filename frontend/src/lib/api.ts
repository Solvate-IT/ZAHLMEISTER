...
export const api = {
  ...
  cancelCollection:(id:string)=>request<T.CollectionSummary>(`/collections/${id}/cancel`,{method:"POST"}),
  ...
  previewCollectionDispatch:(id:string,external_channels:T.CommunicationChannel[])=>request<T.DispatchPreviewResult>(`/collections/${id}/dispatch-preview`,{method:"POST",body:JSON.stringify({external_channels})}),
  ...
  createTemplate:(payload:{name:string;body?:string;source_language?:string})=>request<T.MessageTemplate>("/message-templates",{method:"POST",body:JSON.stringify(payload)}),
  ...
  billingPurchaseContext:(provider:"apple"|"google"|"mollie")=>request<T.BillingPurchaseContext>(`/billing/purchase-context?provider=${encodeURIComponent(provider)}`),
  googlePlayBillingConfig:()=>request<T.GooglePlayBillingConfig>("/billing/google/config"),
  googlePlayBillingVerify:(purchase_token:string)=>request<T.BillingEntitlement>("/billing/google/verify",{method:"POST",body:JSON.stringify({purchase_token})}),
  googlePlayBillingSync:()=>request<T.BillingEntitlement>("/billing/google/sync",{method:"POST"}),
