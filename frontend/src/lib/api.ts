...
export const api = {
  ...
  queueInternalCommunication:(cid:string,pid:string,channel:string,kind="manual")=>request<{message_id:string;status:string}>(`/collections/${cid}/participants/${pid}/communications/internal`,{method:"POST",body:JSON.stringify({channel,kind})}),
