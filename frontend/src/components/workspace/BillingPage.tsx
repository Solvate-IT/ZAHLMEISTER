...
export function BillingPage(){
  ...
  async function startCheckout(){
    if(!profile){setEditingProfile(true);setError(t("billingProfileRequired"));return}
  ...
  async function cancelRenewal(){setBusy(true);setError("");try{const next=await api.mollieBillingCancel();setEntitlement(next);setCancelOpen(false);await load();setNotice(next.expires_at?t("billingCancelledNotice",{date:formatDate(next.expires_at,locale)}):t("billingStatusCancelled"))}catch{setError(t("billingError"))}finally{setBusy(false)}}
  ...
  return <>
    ...
    <div className="split">
      ...
      <section className="card">
        {native?<><h3>{t("billingPro")}</h3><p className="muted">{t("billingNativeStoreHint")}</p></>:active?<><h3>{provider||t("billingPro")}</h3>{entitlement.provider==="mollie"&&entitlement.auto_renew&&<><p className="muted">{renewalDate?t("billingNextRenewal",{date:formatDate(renewalDate,locale),price:renewalPrice}):t("billingProHint")}</p><div className="notice">{t("billingAutoDebitMandate")}</div><button className="button danger" onClick={()=>setCancelOpen(true)} disabled={busy}>{t("billingCancel")}</button></>}{entitlement.provider==="mollie"&&!entitlement.auto_renew&&entitlement.expires_at&&<div className="notice">{t("billingCancelledNotice",{date:formatDate(entitlement.expires_at,locale)})}</div>}</>:pending?<><h3>{t("billingReturnProcessing")}</h3><p className="muted">{t("billingProHint")}</p><div className="actions"><button className="button secondary" onClick={refresh} disabled={busy}>{t("billingRefresh")}</button><button className="button" onClick={startCheckout} disabled={busy}>{t("billingUpgrade")}</button></div></>:config.available?<><h3>{t("billingPro")}</h3><p><strong>{t("billingPriceYear",{price})}</strong></p><div className="row between"><span className="muted">{t("billingRenews")}</span><strong>{t("billingYes")}</strong></div><div className="notice">{t("billingAutoDebitMandate")}</div><p className="muted">{t("billingProHint")}</p>{config.environment==="test"&&<div className="notice"><strong>{t("billingTestMode")}</strong><div>{t("billingTestModeHint")}</div></div>}<button className="button" onClick={startCheckout} disabled={busy||!profile}>{busy?t("billingOpeningCheckout"):t("billingUpgrade")}</button>{!profile&&<p className="muted">{t("billingProfileRequired")}</p>}</>:<><h3>{t("billingUnavailable")}</h3><p className="muted">{t("billingUnavailableHint")}</p></>}
      </section>
    </div>

    {showMollieDetails&&<section className="card">
      <div className="row between"><div><h3>{t("billingDetailsTitle")}</h3><p className="muted">{t("billingDetailsHint")}</p></div>{profile&&!editingProfile&&<button className="button secondary" onClick={()=>setEditingProfile(true)}>{t("billingDetailsEdit")}</button>}</div>
